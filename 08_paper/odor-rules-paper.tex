...
\begin{figure}[pos=H]
\centering
\includegraphics[width=\linewidth]{fig3_composite_dashboard.png}
\caption{Composite empirical dashboard synthesizing eight complementary views of the odor transformation space. (A) Source and target odor category distributions (weighted frequency). (B) Pathway length distribution showing the dominance of multi-step EC sequences. (C) Source$\to$target odor transformation heatmap (log-scaled weighted frequency). (D) Top 20 EC sequences color-coded by enzyme functional class. (E) EC sequence usage stratified by source odor category. (F) EC sequence usage stratified by target odor category. (G) EC functional class distribution normalized by pathway length. (H) Major odor transformation flows (line width $\propto$ frequency).}
